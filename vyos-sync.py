        ...
        for line in fd:
            if cnt_start:
                ...
                if filename.startswith(f"{repo}/binary-{arch}") or \
                   filename.startswith(f"{repo}/Contents-{arch}.gz") or \
                   filename.startswith(f"Release") or \
                   filename.startswith(f"InRelease") or \
                   filename.startswith(f"Contents-{arch}.gz"): 
                    fn = Path(filename)
...
def main():
    ...
    parser.add_argument("os_version", type=str, help="e.g. current")
    parser.add_argument("component", type=str, help="e.g. main")
    parser.add_argument("arch", type=str, help="e.g. amd64")
    ...
    for os in os_list:
        for comp in component_list:
            for arch in arch_list:
                download_base_url=f'{args.base_url}/repositories/{os}'
                if apt_mirror(download_base_url, os, comp, arch, args.working_dir, deb_set=deb_set) != 0:
    ...
    current_url=f'{args.base_url}/repositories/current'
    
    apt_mirror(current_url, 'current', 'main', 'amd64', args.working_dir, deb_set=deb_set)
